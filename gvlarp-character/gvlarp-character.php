...
function gvlarp_character_install() {
    ...
    $gvlarp_character_db_version = "1.6.0";

    $current_table_name = $table_prefix . "PLAYER_TYPE";
    if ($wpdb->get_var("SHOW TABLES LIKE '$current_table_name'") != $current_table_name) {
...
function print_character_temp_stats($atts, $content=null) {
    ...
    if ($sqlOutput != "") {
        if ($stat == "Willpower") {
            $output = "<table class=\"gvplugin\" id=\"gvid_ctw\"><tr><td class=\"gvcol_1 gvdot_10_" . $sqlOutput . "\" /></tr></table>";
        }
        else if ($stat == "Blood") {
            $output = "<table class=\"gvplugin\" id=\"gvid_ctb\"><tr><td class=\"gvcol_1 gvcol_val\">". $sqlOutput . "</td></tr></table>";
        }
    }
    else {
        $output = "";
...
function print_master_temp_stat_table($atts, $content=null) {
    ...
    foreach ($tempStatReasons as $reason) {
        ...
        if ($monthlyWP && $reason->name == "Monthly WP Gain") {
